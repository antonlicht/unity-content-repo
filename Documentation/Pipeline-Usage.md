...
- **Runtime: `The AssetBundle '…_monoscripts_…' / '…_unitybuiltinassets_…' can't be loaded because another AssetBundle with the same files is already loaded`** — a content package is DLC that runs inside the full client, which already loads identical shared bundles; two catalogs shipping the same shared objects collide (a known Addressables multi-catalog limitation). Content still plays (objects resolve from the client's copy), but the errors are logged. Fixed in 0.3.18–0.3.19: the content build (a) **suppresses its MonoScript bundle** (`MonoScriptBundleNaming = Custom` with an empty prefix — the client's assemblies provide those types) and (b) gives its **built-in-shaders bundle a unique internal name** (`BuiltInBundleNaming = Custom`, prefix `contentrepo_<package>`) so it no longer collides with the client's `ProjectName`-named one. **Rebuild and re-upload any package built before 0.3.19.**
